<script lang="ts">
  ...
  import type { Subject, Topic, GradeCurriculumItem as GCItem } from '$lib/api/types';
  ...
  // Expanded subjects in the master panel
  let expandedSubjects = $state<Set<string>>(new Set());

  // DnD items for the curriculum panel (right side)
  interface DndItem {
    id: string | number;
    subjectId?: string;
    topicId?: string;
  ...
  // Build master items for the source panel
  function buildMasterItems(): DndItem[] {
    if (!data.subjects) return [];
    const items: DndItem[] = [];
    for (const subject of data.subjects) {
      items.push({
        id: `master-subject-${subject.id}`,
        subjectId: subject.id,
        displayName: `${subject.name} (all topics)`,
        type: 'subject'
      });
      if (expandedSubjects.has(subject.id)) {
        for (const topic of subject.topics ?? []) {
          items.push({
            id: `master-topic-${topic.id}`,
            topicId: topic.id,
  ...
  let masterItems = $derived(buildMasterItems());

  // Check if an item is already in the curriculum
  function isInCurriculum(item: DndItem): boolean {
    return curriculumItems.some(
      ci => (item.subjectId && ci.subjectId === item.subjectId) ||
            (item.topicId && ci.topicId === item.topicId)
    );
  }
  ...
  // Add item to curriculum (click-to-add)
  function addToCurriculum(item: DndItem) {
    if (isInCurriculum(item)) return;
  ...
  // DnD handlers for curriculum zone (right panel)
...
<div>
  <div class="flex items-center justify-between mb-6">
    <div>
      ...
      <p class="text-sm text-text-muted mt-1">Drag subjects and topics from the master curriculum to build the yearly plan.</p>
    </div>
    <a href="/school/curriculum" class="text-sm text-primary hover:underline">
  ...
  {#if saveSuccess}
    <div class="mb-4">
      <Alert variant="success">Curriculum saved successfully!</Alert>
    </div>
  {/if}
  ...
  {:else if data.academicYears.length === 0}
    <div class="text-center py-12 text-text-muted">
      ...
      <p class="text-sm mt-1">
        <a href="/school/settings/academic-years" class="text-primary hover:underline">Create an academic year</a> to get started.
      </p>
    </div>
  {:else}
    <!-- Selectors -->
              ...
              {#each data.subjects as subject}
                ...
                <div>
                  ...
                  <div class="flex items-center gap-2 p-2 rounded-lg hover:bg-slate-50 group {inCurriculum ? 'opacity-50' : ''}">
                    <button
                      class="text-text-muted hover:text-text transition-transform {isExpanded ? 'rotate-90' : ''}"
                    ...
                    <button
                      class="opacity-0 group-hover:opacity-100 text-xs bg-primary text-white px-2 py-1 rounded transition-opacity disabled:opacity-30"
                      onclick={() => addToCurriculum({ id: `master-subject-${subject.id}`, subjectId: subject.id, displayName: `${subject.name} (all topics)`, type: 'subject' })}
                    ...
                    <div class="ml-6 space-y-0.5">
                      {#each subject.topics ?? [] as topic}
                        ...
                        <div class="flex items-center gap-2 p-1.5 pl-3 rounded hover:bg-slate-50 group {topicInCurriculum ? 'opacity-50' : ''}">
                          ...
                          <button
                            class="opacity-0 group-hover:opacity-100 text-xs bg-primary/80 text-white px-2 py-0.5 rounded transition-opacity disabled:opacity-30"
                            onclick={() => addToCurriculum({ id: `master-topic-${topic.id}`, topicId: topic.id, displayName: topic.name, type: 'topic', subjectName: subject.name })}
                            disabled={topicInCurriculum}
                          >
                            + Add
                          </button>
                        </div>
      ...
      <div>
        ...
        >
          ...
          {#each curriculumItems as item, index (item.id)}
            <div
              class="flex items-center gap-3 p-3 mb-2 bg-white rounded-lg border border-slate-100 shadow-sm cursor-grab active:cursor-grabbing"
            ...
            >
              ...
              <!-- Type badge -->
              <span class="text-xs px-2 py-0.5 rounded-full flex-shrink-0 {item.type === 'subject' ? 'bg-indigo-50 text-indigo-600' : 'bg-emerald-50 text-emerald-600'}">
                {item.type}
              </span>
              <!-- Remove button -->
              <button
                class="text-slate-300 hover:text-red-500 transition-colors flex-shrink-0"
                onclick={() => removeFromCurriculum(index)}
                aria-label="Remove"
              >
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
