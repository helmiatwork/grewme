<script lang="ts">
  ...
  function creatorLabel(type: string): string {
    if (type === 'SchoolManager') return '🏫 School';
  ...
  async function handleCreate(e: SubmitEvent) {
    ...
    if (!formClassroomId || !formTitle.trim() || !formDate) {
      formError = m.calendar_required_error();
      return;
    }
    submitting = true;
    formError = '';
...
<svelte:head>
  <title>School Calendar — GrewMe</title>
</svelte:head>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    <div>
      <h2 class="text-lg font-semibold text-text mb-3">
        {#if selectedDay !== null}
          {m.calendar_events_for({ month: MONTH_NAMES[viewMonth], day: String(selectedDay) })}
          <button
            type="button"
      ...
      <form onsubmit={handleCreate} class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          <label for="modal-startTime" class="block text-sm font-medium text-text mb-1">
            {m.calendar_start_time()} <span class="text-text-muted font-normal">({m.feed_optional()})</span>
          </label>
          <input
            id="modal-startTime"
            type="time"
        ...
        <div>
          <label for="modal-endTime" class="block text-sm font-medium text-text mb-1">
            {m.calendar_end_time()} <span class="text-text-muted font-normal">({m.feed_optional()})</span>
          </label>
          <input
            id="modal-endTime"
            type="time"
        ...
        <div class="sm:col-span-2">
          <label for="modal-description" class="block text-sm font-medium text-text mb-1">
            {m.calendar_description_label()} <span class="text-text-muted font-normal">({m.feed_optional()})</span>
          </label>
          <textarea
