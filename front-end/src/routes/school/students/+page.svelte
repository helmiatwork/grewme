...
<div>
  <h1 class="text-2xl font-bold text-text mb-6">{m.school_students_title()}</h1>

  {#if form?.error}
    <div class="mb-4"><Alert variant="error">{form.error}</Alert></div>
  {/if}
  {#if form?.success}
    <div class="mb-4"><Alert variant="success">{m.school_students_transferred()}</Alert></div>
  {/if}
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      {#each data.classrooms as classroom}
        <div class="mb-6">
          ...
          {#if classroom.students.length === 0}
            <p class="text-sm text-text-muted">{m.school_students_no_enrolled()}</p>
          {:else}
    ...
    <div>
      <h2 class="text-lg font-semibold text-text mb-3">{m.school_students_transfer()}</h2>
      <Card>
        {#snippet children()}
          <form method="POST" action="?/transfer" use:enhance class="space-y-3">
            <div>
              <label for="fromClassroomId" class="block text-sm font-medium text-text mb-1">{m.school_students_from_classroom()}</label>
              <select name="fromClassroomId" id="fromClassroomId" bind:value={transferFromId} class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" required>
                <option value="">{m.school_students_select_classroom()}</option>
            ...
            <div>
              <label for="studentId" class="block text-sm font-medium text-text mb-1">{m.school_students_student_select()}</label>
              <select name="studentId" id="studentId" bind:value={transferStudentId} class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" required>
                <option value="">{m.school_students_select_student()}</option>
            ...
            <div>
              <label for="toClassroomId" class="block text-sm font-medium text-text mb-1">{m.school_students_to_classroom()}</label>
            ...
            <Button type="submit" class="w-full">{m.school_students_transfer_btn()}</Button>
          </form>
        {/snippet}
      </Card>
    </div>
