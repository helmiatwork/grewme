...
export const actions: Actions = {
  ...
  deleteTopic: async ({ locals, cookies, params }) => {
    ...
    try {
      ...
      throw redirect(303, `/teacher/curriculum/${subjectId}`);
    } catch (err) {
  ...
  updateLearningObjective: async ({ request, locals, cookies }) => {
    ...
    try {
      ...
      if (data.updateLearningObjective.errors?.length) {
        return fail(422, { loEditError: data.updateLearningObjective.errors.join(', ') });
      }

      return { loEditSuccess: true };
    } catch (err) {
      if (err instanceof GraphQLError && err.message.toLowerCase().includes('authentication')) {
        clearAuthCookies(cookies);
        throw redirect(303, '/login?force');
      }
      return fail(500, { loEditError: 'Failed to update learning objective' });
    }
  },

  deleteLearningObjective: async ({ request, locals, cookies }) => {
    const formData = await request.formData();
    const id = formData.get('id') as string;
