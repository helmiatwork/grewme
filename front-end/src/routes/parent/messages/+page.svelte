<script lang="ts">
  ...
  // ── State ──────────────────────────────────────────────────────────────────
  type ActiveChat = { type: 'direct' | 'group'; id: string } | null;
  ...
  const isGroupChat = $derived(activeChat?.type === 'group');

  // ── Derived sidebar data ───────────────────────────────────────────────────
  const filteredGroupChats = $derived(
    (data.groupConversations ?? []).filter((gc: any) =>
      (gc.classroom?.name ?? gc.name ?? '').toLowerCase().includes(searchQuery.toLowerCase())
    )
  );
  ...
  async function selectGroupChat(gc: any) {
    activeChat = { type: 'group', id: gc.id };
    messages = [];
    loading = true;
    const res = await gql(GROUP_MESSAGES_QUERY, { id: gc.id });
        ...
        {#each filteredGroupChats as gc}
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              <p class="font-semibold text-text text-sm truncate">{gc.classroom?.name ?? gc.name}</p>
              {#if gc.lastMessage}
      ...
      {#if filteredDirectChats.length > 0}
        ...
        {#each filteredDirectChats as conv}
          ...
          >
            ...
            >
              {conv.teacher?.name?.charAt(0)?.toUpperCase() ?? '?'}
            </div>
            <!-- Info -->
            <div class="flex-1 min-w-0">
              <p class="font-semibold text-text text-sm truncate">
                {conv.teacher?.name ?? 'Unknown Teacher'}
              </p>
    ...
    {:else}
      ...
      <header class="bg-white border-b border-gray-200 px-6 py-4 flex items-center gap-3 shadow-sm flex-shrink-0">
        {#if activeChat.type === 'group'}
          {@const gc = (data.groupConversations ?? []).find((g: any) => g.id === activeChat?.id)}
          <div
      ...
      {#if isGroupChat}
        <!-- Read-only banner for group chats -->
        <div class="bg-gray-100 border-t border-gray-200 px-4 py-3 text-center text-sm text-text-muted flex-shrink-0">
          Only teachers can send messages in group chats
        </div>
      {:else}
        <!-- Attachment previews -->
