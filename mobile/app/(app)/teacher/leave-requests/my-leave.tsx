...
export default function MyLeaveTab() {
  ...
  const renderRequest = ({
    ...
    return (
      <View style={s.requestCard} testID={`leave-item-${item.id}`}>
        ...
        <View style={s.requestDetails}>
          ...
          {item.halfDaySession ? (
            <View style={s.detailRow}>
              ...
              <Text style={s.detailText}>Half day ({item.halfDaySession})</Text>
            </View>
          ) : null}
          <View style={s.detailRow}>
            <Ionicons
  ...
  return (
    <View style={s.container} testID="teacher-my-leave-tab">
      {/* Balance Card */}
      {balance ? (
      ...
      {/* List / Empty State */}
      {requests.length === 0 ? (
        <View style={s.emptyState} testID="leave-empty-state">
