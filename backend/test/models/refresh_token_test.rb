...
class RefreshTokenTest < ActiveSupport::TestCase
  test "generates token digest on create" do
    token = teachers(:teacher_alice).refresh_tokens.create!
  ...
  test "expired returns true after expires_at" do
    token = teachers(:teacher_alice).refresh_tokens.create!(expires_at: 1.day.ago)
    assert token.expired?
  end

  test "active scope excludes revoked and expired" do
    active = teachers(:teacher_alice).refresh_tokens.create!
    revoked = teachers(:teacher_alice).refresh_tokens.create!
    revoked.revoke!
    expired = teachers(:teacher_alice).refresh_tokens.create!(expires_at: 1.day.ago)
